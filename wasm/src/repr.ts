...
//
import { t, N, AnyOp, VarUint7, OpCode }                   from './ast'
...
const reprop = (op: OpCode) => style(opcodes[op], '96')
...
export function BufferedWriter(): Writer {
  const buf: string[] = [];
  const writer = (s: string) => { buf.push(s) };
